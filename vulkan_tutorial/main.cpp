...
struct QueueFamilyIndices {
    // there is no value to indicate a non-existent queue family, therefore we use optional
...
private:
    ...
    void createLogicalDevice() {
        ...
        // we don't need anything special for now, so everything can be left to VK_FALSE
        VkPhysicalDeviceFeatures deviceFeatures{};
    ...
    void createInstance() {
        ...
        // Moved outside of the if statement to ensure that it's not destroyed before vkCreateInstance call.
        ...
        if (vkCreateInstance(&createInfo, nullptr, &instance_) != VK_SUCCESS) {
            ...
            std::cout << "Instance for application " << appInfo.pApplicationName << " created successfully" << std::endl;
        }
    }
    ...
    // returns a boolean indicating if the call that triggered the validation layer should be aborted
    static VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
            // might be diagnostic/informational/possible_bug/error
            VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
            // unrelated to the specification or performace/something that violates the specification/potential non-optimal use of Vulkan
            VkDebugUtilsMessageTypeFlagsEXT messageType,
            // details of the message itself: message + array of related object handles
    ...
    VkDevice device_;
    // handle to interface with the queues which are created along with the logical device
    VkQueue graphicsQueue_;
    // won't be platform agnostic, thankfully it can be created with glfwCreateWindowSurface that can handle the platform differences
