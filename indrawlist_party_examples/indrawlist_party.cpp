...
void ShowEffectsWindow() {
  ...
  ImGui::Begin("FX", nullptr, ImGuiWindowFlags_AlwaysAutoResize);
  static const char* selected_effect = effects::effects_map.begin()->first;
  if (ImGui::BeginCombo("choose effect", selected_effect, 0))
  {
    for (const auto& [key, value] : effects::effects_map) {
      const bool is_selected = (selected_effect == key);
  ...
  ImGui::BeginChild("FX", ImVec2(320.0f, 180.0f), false, ImGuiWindowFlags_AlwaysAutoResize);
  ...
  effects::effects_map.at(selected_effect)(draw_list, p0, p1, size, mouse_data, (float)ImGui::GetTime());
...
int main(int, char**)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 2. Show a window that displays imdrawlist effects.
        // Testbed from indrawlist_party: https://github.com/ocornut/imgui/issues/3606
       ShowEffectsWindow();
